...
'use strict';
function SubjectConfigCtrl($scope,$state, $stateParams, SubjectConfigService,$timeout,SweetAlert,toaster,$q) {
    var vm = this;
    ...
    $scope.$parent.vm.stateName = "Define Subject Datasets";
    ...
    if($stateParams.activityId==0){
        ...
        activity = new SubjectConfigService.getActivityResource();
        activity.projectId = $stateParams.projectId;//"Study1"
        activity.isNew = true;
        activity.status = "New";
        activity.datasets = [];
        activity.activityId = 0;
        activity.name = "Subject Data Collection"

        vm.activity = activity;
        SubjectConfigService.getDatasetResource.query(function(response){
        ...
        SubjectConfigService.getActivityResource.get({ activityId: $stateParams.activityId }, function(response){
            activity = response;
...
angular.module('bioSpeak.config')
    .controller('SubjectConfigCtrl',['$scope','$state','$stateParams','SubjectConfigService','$timeout','SweetAlert','toaster','$q',SubjectConfigCtrl]);
