 ...
 */
'use strict'
function AssayConfigCtrl($scope, $state, $stateParams, ActivityResource, DatasetResource, AssayConfigService,$timeout){
    var vm = this;
    vm.studyId = $stateParams.studyId
    console.log($stateParams.studyId)
    ...
    if($stateParams.assayId==0){
        ...
        vm.assay.ProjectAcc = $stateParams.studyId;//"Study1"
        vm.assay.isNew = true;
        vm.assay.status = "New";
        vm.assay.datasets = {};
        vm.assay.assayId = 0;
    ...
    vm.showTemplate = function(domainId,dsType){
        DatasetResource.get({datasetId:domainId}, function(response) {
            ...
            vm.selectedDatasets[dsType].activityId = $stateParams.activityId;
...
angular.module('bioSpeak.config')
    .controller('AssayConfigCtrl',['$scope', '$state','$stateParams','ActivityResource','DatasetResource','AssayConfigService','$timeout',AssayConfigCtrl])
